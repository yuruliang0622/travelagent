...
class TripRepository:
    ...
    def vector_search_memory(self, query: str, limit: int = 6) -> list[MemorySearchResult]:
        """Semantic search using MongoDB Atlas Vector Search and Gemini embeddings.

        Falls back to an empty list when Atlas is unavailable or embedding fails.
        """
        if not self._mongo_available or not query.strip():
            return []
        ...
        collections_to_search = [
            (self._destination_packs_collection(), "destination"),
            (self._trips_collection(), "itinerary"),
        ]

        for collection, scope in collections_to_search:
            try:
                pipeline = [
                    {
                        "$vectorSearch": {
                            "index": self._settings.mongodb_vector_index,
                ...
                for doc in docs:
                    ...
                    title = str(doc.get("title") or doc.get("country") or doc_id)
                    notes = doc.get("cultural_notes", [])
                    snippet = " · ".join(str(n) for n in notes[:3]) if notes else title
                    results.append(
                        MemorySearchResult(
                            id=f"{scope}:{doc_id}",
                            scope=scope,
